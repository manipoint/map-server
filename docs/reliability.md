...
| P2 | Search snapshots and provider evidence are not implemented | Saved itinerary timelines remain available, but live price/availability evidence cannot be audited or refreshed from a durable snapshot. | Trips and versioned saved itineraries are persisted. | Implement bounded normalized evidence snapshots before promising durable price or availability history. |
